...
local update_host = function()
	awful.spawn.easy_async_with_shell(
		[[bash -c "upower -i $(upower -e | grep BAT) | grep 'state' | sed 's/state://'"]],
		function(stdout)
			local batteryChargeStatus = stdout:gsub(" ", "")
			right_content:set_text(batteryChargeStatus)
		end
	)
end
